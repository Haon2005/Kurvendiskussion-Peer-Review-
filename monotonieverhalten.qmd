...
Die Monotonie kann, wie in dem Kapitel **[Ableitungen](ableitungen.qmd)** angedeutet wurde, mithilfe der Ableitung bestimmt werden, da diese an jedem Punkt die Steigung an dieser Stelle darstellt.
...
Die Nullstellen von $f'(x)$ teilen den Definitionsbereich in die Intervalle von $-\infty$ bis $F(x_0)$, von $F(x_0)$ bis $F(x_1)$, $\dots$ und von $F(x_{n-1})$ bis $\infty$, wobei $x_0$ bis $x_{n-1}$ die Nullstellen von $f'(x)$ sind.
...
Falls zwei oder mehr aufeinanderfolgende Intervalle steigend/fallend sind, so können sie als ein nicht *streng* monoton, sondern nur monoton steigendes/fallendes Intervall betrachtet werden.

# Python Implementierung

Nun geht es darum, diesen ganzen Prozess als Python Programm zu automatisieren.
Zuerst einmal brauchen wir die Erkenntnisse, die du in den **[Nullstellen](nullstellen.qmd)**, sowie **[Ableitungen](ableitungen.qmd)** Kapiteln gelernt hast.
...
Jetzt geht es darum den letzten Schritt des Programms zu schreiben.
...
`For-Schleifen`wiederholen sich über einen gewisses Intervall. Das Schlüsselwort `range` beschreibt hierbei das Intervall von $a$ bis $b$. Dabei ist $a$ der Start der Iteration und $b$ der Endpunkt der Iteration.

```{pyodide-python}
a = 5 
b = 20
for i in range(a,b,2): #a und b sind die Grenzen, 2 die Schrittgröße
    print(i+1)
```
...
Versuch das obige Programm (hier drunter einmal in Kopie) so fortzusetzen, dass es die Liste durchgeht und in jedem Intervall einen Wert testet, dessen Vorzeichen bestimmt (also ob die Funktion steigt/fällt) und das nutzt, um das Monotonieverhalten passend zu bestimmen. 
...
Baue in das Programm noch eine Ausgabe ein, welche Nutzenden des Programms ausgibt, was das Monotonieverhalten ist.  
    
Hier kannst du an der Ausgabe noch eine Menge Stellschrauben drehen, die die Ausgabe direkter machen. Das heißt, wie oben erwähnt, kannst du Abschnitte an denen das gleiche Vorzeichen nacheinander vorkommt zusammenfassen und statt _streng_ monoton, als **monoton** steigend/fallend bezeichnen.  
...
Wenn du mit deinem Ergebnis zufrieden bist, geht es weiter in dem Unterkapitel**[Extrempunkte-Sattelpunkte](extrempunkte.qmd)**.